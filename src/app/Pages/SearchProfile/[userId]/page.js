...
const SearchProfile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        // Fetch user profile - note the collection name matches your Firestore
        const userDoc = await getDoc(doc(db, "users", userId)); // Changed from "Users" to "users"
  ...
  const handleFollow = async () => {
    ...
    try {
      const currentUserRef = doc(db, "users", currentUser.uid); // Changed to "users"
      const profileUserRef = doc(db, "users", userId); // Changed to "users"
